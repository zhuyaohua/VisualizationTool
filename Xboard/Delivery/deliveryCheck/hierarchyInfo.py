...
def hierarchyInfo():
    cmd = """
        SELECT rule_value.rule_code,rr.`审查项`,rr.type,rr.`专业`,rr.`城市`,rr.`规范条目数` FROM
    ...
    WHERE rule_param.param_value_id = "41415" AND rule_value.rule_lib_id in (SELECT id FROM cbim_rule.rule_lib WHERE lib_code in ("ZNSC","SZGC-DH","ZYTZ","JSGL-DH-1","JGSC-DH") AND lib_status =0)
    ...
    UNION
    SELECT rule_value.rule_code,rr.`审查项`,"" AS type,rr.`专业`,rr.`城市`,rr.`规范条目数` FROM
    (SELECT rule_value.rule_value AS "审查项",oo.`专业`,oo.`城市`,oo.`规范条目数`,hierarchy_class_lib.parent_line_num FROM 
    (SELECT ll.`专业`,ll.`城市`,rule_value.rule_value AS "规范条目数",ll.line_num FROM
    (SELECT nn.`专业`,rule_value.rule_value AS "城市",nn.line_num FROM
    (SELECT rule_value.rule_value AS "专业",rule_value.line_num FROM rule_value
    LEFT JOIN rule_param ON rule_param.id = rule_value.head_id
    WHERE rule_param.param_value_id = "18236" and rule_value.rule_lib_id in (SELECT id FROM cbim_rule.rule_lib WHERE lib_code = "GH-DH" AND lib_status =0)) nn
    LEFT JOIN rule_value ON rule_value.line_num = nn.line_num
    LEFT JOIN rule_param ON rule_param.id = rule_value.head_id
    WHERE rule_param.param_value_id = "43983") ll
    LEFT JOIN rule_value ON rule_value.line_num = ll.line_num
    ...
    datas.to_excel("规则引擎数据-审查项(全).xls", index=None)
